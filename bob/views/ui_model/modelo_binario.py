"""
    ...
    History:
        20201.01    Script first version, template method file
                    LerArquivoBinario, EscreverArquivoBinario.
...
class LerArquivoBinario(LerArquivo):
    def __init__(self, arquivo):
        self.arquivo = Path(arquivo)
        self.resultado = None

    def ler_conteudo(self):
        self.resultado = self.arquivo.read_bytes()

    def depois_de_ler(self):
        """Limpar dados."""   # import pdb; pdb.set_trace()
        self.resultado = self.resultado.replace(b'\n', b'')
...
class EscreverArquivoBinario(EscreverArquivo):
    def __init__(self, arquivo, conteudo):
        self.arquivo = Path(arquivo)
        self.conteudo = conteudo

    def escrever_conteudo(self):
        self.arquivo.write_bytes(self.conteudo)
